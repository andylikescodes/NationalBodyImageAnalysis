...
for key in data_sets.keys():
	...
	X_cat = df[cat_demo]
	woman_index = X_cat['Sex']==0
	X_cat = X_cat.loc[woman_index, :].astype('object')
	X_cat_dummies = pd.get_dummies(X_cat, drop_first=True)
	X_num = df.loc[woman_index, num_demo]
	X_survey = df.loc[woman_index, data_sets[key]]
	ys = df.loc[woman_index, y_variables]
	...
	output.to_csv('../outputs/' + 'woman_' + key + '.csv')
